/*
  ...
  OPTIONAL PARAMETERS:
    ...
    - center [true/false] = true: for small width should slider be centered or left aligned?
...
class HorizontalSlider {
  ...
  constructor(componentEl, data, renderFunction, wrap = false, center = true, threshold = 100, index = 0, 
              createClickHandlers = null, unSelectHandlers = null, forwardBack = false) {
  ...
  init() {
    ...
    const columnWidth = this.slidegroup.computedStyleMap().get('grid-auto-columns').value;    
    this.slideSize = columnWidth / 100;    
    ...
    if (this.allowWrap) {
      ...
      cloneFirstSlide.style.backgroundColor = "gray";
      cloneLastSlide.style.backgroundColor = "slategrey";
  ...
  transitionEnd() {  
    ...
    //console.log('slide width: ', this.slideWidth, this.slideSize);
    ...
    if (this.allowWrap) {
      ...
      if (this.selectedIndex < MIN_INDEX) {   
        this.slidegroup.style.left = -(this.maxLeft + this.slideWidth) +  "px";     
        newIndex = MAX_INDEX;        
      }

      // Edge case Right:  selected index is at last element in array.  Send it to first element.
      if (this.selectedIndex > MAX_INDEX) {                
        this.slidegroup.style.left = -(this.slideWidth) + this.slidePadding + "px";        
        newIndex = 1;   
      }   
      
      //console.log(this.slidegroup.style.left);
